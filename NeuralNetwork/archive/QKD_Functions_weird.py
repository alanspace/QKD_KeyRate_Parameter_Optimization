...
def penalty(key_rates, mu_1, mu_2, mu_3, P_mu_1, P_mu_2, P_mu_3):
    ...
    penalty_mu1_sum = jnp.where(mu_1 > mu_2 + mu_3, 0.0, 1e20)
    penalty_mu2_ratio = jnp.where(mu_2 / mu_1 < 1, 0.0, 1e20)
    ...
    penalty_sum = jnp.where(jnp.abs(P_mu_1 + P_mu_2 + P_mu_3 - 1) < 1e-6, 0.0, 1e20)
    penalty_P_mu_3 = jnp.where(P_mu_3 > 0, 0.0, 1e20)
    penalty_mu2_mu3 = jnp.where(mu_2 > mu_3, 0.0, 1e20)
    # Sum all penalties
