...
def experimental_parameters(device="cpu"):
    """
...
def other_parameters(device="cpu"):
    # other parameters
    alpha = torch.tensor(0.2, device=device, dtype=torch.float32)  # Attenuation coefficient (dB/km)
    eta_Bob = torch.tensor(0.1, device=device, dtype=torch.float32)  # Detector efficiency
    P_ap = torch.tensor(1e-6, device=device, dtype=torch.float32)  # After-pulse probability
    f_EC = torch.tensor(1.16, device=device, dtype=torch.float32)  # Error correction efficiency
    epsilon_sec = torch.tensor(1e-10, device=device, dtype=torch.float32)  # Security error
    epsilon_cor = torch.tensor(1e-15, device=device, dtype=torch.float32)  # Correlation error
    n_event = torch.tensor(1, device=device, dtype=torch.float32)  # For single photon event
    kappa = torch.tensor(1e-15, device=device, dtype=torch.float32)  # Security parameter
...
Calculate the factorial using the gamma function.
    ...
    """
    return torch.lgamma(torch.tensor(n + 1.0, dtype=torch.float32)).exp()
...
#     """
#     eta = 10 ** (-alpha * L / 10)
#     # return torch.clamp(torch.tensor(eta, dtype=torch.float32), 1e-6, 1.0)
#     return torch.clamp(eta.clone().detach().to(torch.float32), 1e-6, 1.0)

def calculate_eta_ch(L, alpha, device="cpu"):
    """
...
def calculate_n_X_total(n_event, mu_1, mu_2, mu_3, P_mu_1, P_mu_2, P_mu_3, P_dc, eta_sys_value, P_X_value, n_X_values):
    ...
    # n_event = torch.tensor(n_event, dtype=torch.float32, device=device)
    # mu_1 = torch.tensor(mu_1, dtype=torch.float32, device=device)
    # mu_2 = torch.tensor(mu_2, dtype=torch.float32, device=device)
    # mu_3 = torch.tensor(mu_3, dtype=torch.float32, device=device)
    # P_mu_1 = torch.tensor(P_mu_1, dtype=torch.float32, device=device)
    # P_mu_2 = torch.tensor(P_mu_2, dtype=torch.float32, device=device)
    # P_mu_3 = torch.tensor(P_mu_3, dtype=torch.float32, device=device)
    # P_dc = torch.tensor(P_dc, dtype=torch.float32, device=device)
    # eta_sys_value = torch.tensor(eta_sys_value, dtype=torch.float32, device=device)
    # P_X_value = torch.tensor(P_X_value, dtype=torch.float32, device=device)
    # n_X_values = torch.tensor(n_X_values, dtype=torch.float32, device=device)
...
def calculate_e_obs(m_X_total, n_X_values):
    ...
    result = m_X_total / n_X_values
    return torch.minimum(result, torch.tensor(0.5, dtype=torch.float32))
...
def calculate_tau_n(n, mu_k_values, p_mu_k_values):
    """
Calculates Poisson probabilities for intensity levels.
    ...
    """
    mu_k_values = torch.tensor(mu_k_values, dtype=torch.float32)
    p_mu_k_values = torch.tensor(p_mu_k_values, dtype=torch.float32)
...
def calculate_n_pm(mu_k_values, p_mu_k_values, n_mu_k, sqrt_term):
    ...
    n_mu_k = torch.tensor(n_mu_k, dtype=torch.float32, device=device)
    sqrt_term = torch.tensor(sqrt_term, dtype=torch.float32, device=device)

    # Compute n_plus and n_minus
    n_plus = ((torch.exp(mu_k_values)) / p_mu_k_values) * (n_mu_k + sqrt_term)
    n_minus = ((torch.exp(mu_k_values)) / p_mu_k_values) * torch.maximum(n_mu_k - sqrt_term, torch.tensor(0.0, dtype=torch.float32, device=device))
...
def calculate_S_0(tau_0, mu_2, mu_3, n_minus_mu_3, n_plus_mu_2):
    ...
    return torch.maximum(result, torch.tensor(0.0, dtype=torch.float32))
...
def calculate_m_pm(mu_k_values, p_mu_k_values, m_k, sqrt_term):
    ...
    mu_k_values = torch.tensor(mu_k_values, dtype=torch.float32, device=device)
    p_mu_k_values = torch.tensor(p_mu_k_values, dtype=torch.float32, device=device)
    m_k = torch.tensor(m_k, dtype=torch.float32, device=device)
    sqrt_term = torch.tensor(sqrt_term, dtype=torch.float32, device=device)

    # Calculate m_plus and m_minus
    m_plus = (torch.exp(mu_k_values) / p_mu_k_values) * (m_k + sqrt_term)
    m_minus = (torch.exp(mu_k_values) / p_mu_k_values) * torch.maximum(m_k - sqrt_term, torch.tensor(0.0, dtype=torch.float32, device=device))
...
def calculate_l(S_X_0_values, S_X_1_values, binary_entropy_Phi_values, lambda_EC_values, epsilon_sec, epsilon_cor):
    ...
    return torch.maximum(l_value, torch.tensor(0.0, dtype=torch.float32))  # Ensure non-negative key length
...
#  mu_k_values, eta_ch_values, p_mu_k_values, p_1 = mu_1, p_2 = mu_2, p_3 = P_mu_1 = 0.65, p_4 = P_mu_2 = 0.3, p_5 =        P_X_value = 5e-3
    ...
    mu_3 = torch.tensor(2e-4, dtype=torch.float32)
    mu_k_values = torch.tensor([mu_1, mu_2, mu_3], dtype=torch.float32)
    P_mu_3 = 1 - P_mu_1 - P_mu_2
    p_mu_k_values = torch.tensor([P_mu_1, P_mu_2, P_mu_3], dtype=torch.float32)
...
# 1. Channel and system efficiencies
    ...
    # D_mu_k_values = torch.tensor([calculate_D_mu_k(mu_k, eta_sys_values, P_dc_value) for mu_k in mu_k_values], dtype=torch.float32)
    ...
    e_mu_k_values = torch.tensor([calculate_e_mu_k(P_dc_value, e_mis, P_ap, D_mu_k, eta_sys_values, mu_k)
                                   for D_mu_k, mu_k in zip(D_mu_k_values, mu_k_values)], dtype=torch.float32)
    ...
    n_X_mu_k_values = torch.tensor([n_X_mu_1, n_X_mu_2, n_X_mu_3], dtype=torch.float32)
    ...
    n_Z_mu_values = torch.tensor([n_Z_mu_1, n_Z_mu_2, n_Z_mu_3], dtype=torch.float32)
    ...
    m_X_mu_values = torch.tensor(m_X_mu_values, dtype=torch.float32)
    ...
    m_Z_total = torch.sum(torch.tensor(m_Z_mu_values, dtype=torch.float32))
...
def objective(params, L_values, n_X, alpha, eta_Bob, P_dc, epsilon_sec, epsilon_cor, f_EC, e_mis, P_ap, n_event, device="cpu"):
    """
