...
#     return to_device(torch.tensor(data, dtype=dtype, device=device, device=device)

def to_device(tensor, device):
    return tensor.to(device, device=device) if isinstance(tensor, torch.Tensor) else to_device(torch.tensor(tensor, device=device)
...
#     P_dc_value = to_device(torch.tensor(6e-7, device=device, dtype=torch.float32, device=device, device=device)  # Dark count probability
#     Y_0 = P_dc_value
#     e_2 = -torch.log(Y_0)  # Related parameter (negative log of Y_0)

#     # 3. Misalignment error
#     e_mis = to_device(torch.tensor(5e-3, device=device, dtype=torch.float32, device=device, device=device)  # Misalignment error probability
#     e_d = e_mis
...
def other_parameters(device=device):
    # other parameters
    alpha = to_device(torch.tensor(0.2, device=device, dtype=torch.float32, device=device, device=device)  # Attenuation coefficient (dB/km)
    eta_Bob = to_device(torch.tensor(0.1, device=device, dtype=torch.float32, device=device, device=device)  # Detector efficiency
    P_ap = to_device(torch.tensor(1e-6, device=device, dtype=torch.float32, device=device, device=device)  # After-pulse probability
    f_EC = to_device(torch.tensor(1.16, device=device, dtype=torch.float32, device=device, device=device)  # Error correction efficiency
    epsilon_sec = to_device(torch.tensor(1e-10, device=device, dtype=torch.float32, device=device, device=device)  # Security error
    epsilon_cor = to_device(torch.tensor(1e-15, device=device, dtype=torch.float32, device=device, device=device)  # Correlation error
    n_event = to_device(torch.tensor(1, device=device, dtype=torch.float32, device=device, device=device)  # For single photon event
    kappa = to_device(torch.tensor(1e-15, device=device, dtype=torch.float32, device=device, device=device)  # Security parameter
    # Return parameters as a dictionary
    return {
        "alpha": alpha,
        "eta_Bob": eta_Bob,
...
def calculate_factorial(n):
    ...
    """
    return torch.lgamma(to_device(torch.tensor(n + 1.0, dtype=torch.float32, device=device, device=device)).exp()
...
#     """
#     eta = 10 ** (-alpha * L / 10)
#     # return torch.clamp(to_device(torch.tensor(eta, dtype=torch.float32, device=device, device=device), 1e-6, 1.0)
#     return torch.clamp(eta.clone(, device=device).detach().to(torch.float32), 1e-6, 1.0)
...
def calculate_eta_ch(L, alpha, device=device):
    ...
    eta = 10 ** (-alpha * L / 10)
    return torch.clamp(eta, 1e-6, 1.0, device=device)
...
#     """
#     return 1 - (1 - 2 * P_dc, device=device) * torch.exp(-eta_sys_values * mu_k)
...
def calculate_D_mu_k(mu_k, eta_sys_values, P_dc):
    ...
    device = mu_k.device
    return 1 - (1 - 2 * P_dc.to(device, device=device)) * torch.exp(-eta_sys_values.to(device) * mu_k.to(device))
...
def calculate_n_X_total(n_event, mu_1, mu_2, mu_3, P_mu_1, P_mu_2, P_mu_3, P_dc, eta_sys_value, P_X_value, n_X_values):
    ...
    # n_event = to_device(torch.tensor(n_event, dtype=torch.float32, device=device, device=device, device=device)
    # mu_1 = to_device(torch.tensor(mu_1, dtype=torch.float32, device=device, device=device, device=device)
    # mu_2 = to_device(torch.tensor(mu_2, dtype=torch.float32, device=device, device=device, device=device)
    # mu_3 = to_device(torch.tensor(mu_3, dtype=torch.float32, device=device, device=device, device=device)
    # P_mu_1 = to_device(torch.tensor(P_mu_1, dtype=torch.float32, device=device, device=device, device=device)
    # P_mu_2 = to_device(torch.tensor(P_mu_2, dtype=torch.float32, device=device, device=device, device=device)
    # P_mu_3 = to_device(torch.tensor(P_mu_3, dtype=torch.float32, device=device, device=device, device=device)
    # P_dc = to_device(torch.tensor(P_dc, dtype=torch.float32, device=device, device=device, device=device)
    # eta_sys_value = to_device(torch.tensor(eta_sys_value, dtype=torch.float32, device=device, device=device, device=device)
    # P_X_value = to_device(torch.tensor(P_X_value, dtype=torch.float32, device=device, device=device, device=device)
    # n_X_values = to_device(torch.tensor(n_X_values, dtype=torch.float32, device=device, device=device, device=device)
    ...
    P_n_given_mu_1 = (mu_1**n_event * torch.exp(-mu_1, device=device)) / calculate_factorial(n_event)
    P_n_given_mu_2 = (mu_2**n_event * torch.exp(-mu_2, device=device)) / calculate_factorial(n_event)
    P_n_given_mu_3 = (mu_3**n_event * torch.exp(-mu_3, device=device)) / calculate_factorial(n_event)

    # Calculate detection probabilities for each mu under channel condition
    D_mu_1 = 1 - torch.exp(-mu_1 * eta_sys_value, device=device) + 2 * P_dc * torch.exp(-mu_1 * eta_sys_value)
    D_mu_2 = 1 - torch.exp(-mu_2 * eta_sys_value, device=device) + 2 * P_dc * torch.exp(-mu_2 * eta_sys_value)
    D_mu_3 = 1 - torch.exp(-mu_3 * eta_sys_value, device=device) + 2 * P_dc * torch.exp(-mu_3 * eta_sys_value)
...
def calculate_N(n_X, p_mu_k_values, D_mu_k_values, P_X_value):
    ...
    """
    terms = to_device(torch.tensor([p_mu_k * D_mu_k * P_X_value**2 for p_mu_k, D_mu_k in zip(p_mu_k_values, D_mu_k_values, device=device)])
...
def calculate_n_Z_total(N_value, p_mu_k_values, D_mu_k_values, P_Z_value, P_det_mu_values):
    ...
    n_Z_mu_values = to_device(torch.tensor([N_value * D_mu_k * p_mu_k * (P_Z_value, device=device)**2 for p_mu_k, D_mu_k in zip(p_mu_k_values, D_mu_k_values)])
    ...
    P_det_mu_Z_values = to_device(torch.tensor([P_det_mu * (P_Z_value, device=device)**2 for P_det_mu in P_det_mu_values])
...
def calculate_e_mu_k(P_dc, e_mis, P_ap, D_mu_k, eta_sys_values, mu_k):
    ...
    """
    return P_dc + e_mis * (1 - torch.exp(-eta_sys_values * mu_k, device=device)) + P_ap * D_mu_k / 2
...
def calculate_e_obs(m_X_total, n_X_values):
    ...
    result = m_X_total / n_X_values
    return torch.minimum(result, to_device(torch.tensor(0.5, dtype=torch.float32, device=device, device=device))
...
def calculate_h(x):
    ...
    x = torch.clamp(x, 1e-10, 1 - 1e-10, device=device)  # Avoid log(0)
...
def calculate_sqrt_term(n_total, epsilon_sec):
    ...
    """
    epsilon_sec = torch.clamp(epsilon_sec, 1e-10, None, device=device)  # Avoid log(0)
...
def calculate_tau_n(n, mu_k_values, p_mu_k_values):
    ...
    """
    mu_k_values = to_device(torch.tensor(mu_k_values, dtype=torch.float32, device=device, device=device)
    p_mu_k_values = to_device(torch.tensor(p_mu_k_values, dtype=torch.float32, device=device, device=device)

    # Calculate tau values
    tau_values = p_mu_k_values * torch.exp(-mu_k_values, device=device) * (mu_k_values ** n) / torch.exp(torch.lgamma(to_device(torch.tensor(n + 1.0)))
...
def calculate_n_pm(mu_k_values, p_mu_k_values, n_mu_k, sqrt_term):
    ...
    mu_k_values = to_device(torch.tensor(mu_k_values, dtype=torch.float32, device=device, device=device, device=device)
    p_mu_k_values = to_device(torch.tensor(p_mu_k_values, dtype=torch.float32, device=device, device=device, device=device)
    n_mu_k = to_device(torch.tensor(n_mu_k, dtype=torch.float32, device=device, device=device, device=device)
    sqrt_term = to_device(torch.tensor(sqrt_term, dtype=torch.float32, device=device, device=device, device=device)

    # Compute n_plus and n_minus
    n_plus = ((torch.exp(mu_k_values, device=device)) / p_mu_k_values) * (n_mu_k + sqrt_term)
    n_minus = ((torch.exp(mu_k_values, device=device)) / p_mu_k_values) * torch.maximum(n_mu_k - sqrt_term, to_device(torch.tensor(0.0, dtype=torch.float32, device=device, device=device))
...
def calculate_S_0(tau_0, mu_2, mu_3, n_minus_mu_3, n_plus_mu_2):
    ...
    return torch.maximum(result, to_device(torch.tensor(0.0, dtype=torch.float32, device=device, device=device))
...
def calculate_m_pm(mu_k_values, p_mu_k_values, m_k, sqrt_term):
    ...
    mu_k_values = to_device(torch.tensor(mu_k_values, dtype=torch.float32, device=device, device=device, device=device)
    p_mu_k_values = to_device(torch.tensor(p_mu_k_values, dtype=torch.float32, device=device, device=device, device=device)
    m_k = to_device(torch.tensor(m_k, dtype=torch.float32, device=device, device=device, device=device)
    sqrt_term = to_device(torch.tensor(sqrt_term, dtype=torch.float32, device=device, device=device, device=device)

    # Calculate m_plus and m_minus
    m_plus = (torch.exp(mu_k_values, device=device) / p_mu_k_values) * (m_k + sqrt_term)
    m_minus = (torch.exp(mu_k_values, device=device) / p_mu_k_values) * torch.maximum(m_k - sqrt_term, to_device(torch.tensor(0.0, dtype=torch.float32, device=device, device=device))
...
def calculate_gamma(a, b, c, d):
    ...
    """
    if (b == 0) or (b == 1):
        return to_device(torch.tensor(0.0, dtype=torch.float32, device=device, device=device)

    term1 = (c + d, device=device) * (1 - b) * b / (c * d * torch.log(to_device(torch.tensor(2.0)))
...
def calculate_l(S_X_0_values, S_X_1_values, binary_entropy_Phi_values, lambda_EC_values, epsilon_sec, epsilon_cor):
    ...
    return torch.maximum(l_value, to_device(torch.tensor(0.0, dtype=torch.float32, device=device, device=device))  # Ensure non-negative key length
...
#  mu_k_values, eta_ch_values, p_mu_k_values, p_1 = mu_1, p_2 = mu_2, p_3 = P_mu_1 = 0.65, p_4 = P_mu_2 = 0.3, p_5 =        P_X_value = 5e-3
    ...
    mu_3 = to_device(torch.tensor(2e-4, dtype=torch.float32, device=device, device=device)
    mu_k_values = to_device(torch.tensor([mu_1, mu_2, mu_3], dtype=torch.float32, device=device, device=device)
    P_mu_3 = 1 - P_mu_1 - P_mu_2
    p_mu_k_values = to_device(torch.tensor([P_mu_1, P_mu_2, P_mu_3], dtype=torch.float32, device=device, device=device)
...
# 1. Channel and system efficiencies
    ...
    # D_mu_k_values = to_device(torch.tensor([calculate_D_mu_k(mu_k, eta_sys_values, P_dc_value, device=device) for mu_k in mu_k_values], dtype=torch.float32, device=device)
    ...
    e_mu_k_values = to_device(torch.tensor([calculate_e_mu_k(P_dc_value, e_mis, P_ap, D_mu_k, eta_sys_values, mu_k, device=device)
    ...
    n_X_mu_k_values = to_device(torch.tensor([n_X_mu_1, n_X_mu_2, n_X_mu_3], dtype=torch.float32, device=device, device=device)
    ...
    n_Z_mu_values = to_device(torch.tensor([n_Z_mu_1, n_Z_mu_2, n_Z_mu_3], dtype=torch.float32, device=device, device=device)
    ...
    m_X_mu_values = to_device(torch.tensor(m_X_mu_values, dtype=torch.float32, device=device, device=device)
    ...
    m_Z_total = torch.sum(to_device(torch.tensor(m_Z_mu_values, dtype=torch.float32, device=device, device=device))
...
def penalty(key_rates, mu_1, mu_2, mu_3, P_mu_1, P_mu_2, P_mu_3):
    ...
    # Compute penalties
    penalty_mu1_sum = torch.where(mu_1 > mu_2 + mu_3, to_device(torch.tensor(0.0, device=device, device=device), to_device(torch.tensor(1e6, device=device))
    penalty_mu2_ratio = torch.where(mu_2 / mu_1 < 1, to_device(torch.tensor(0.0, device=device, device=device), to_device(torch.tensor(1e6, device=device))
    penalty_sum = torch.where(torch.abs(P_mu_1 + P_mu_2 + P_mu_3 - 1, device=device) < 1e-6, to_device(torch.tensor(0.0, device=device), to_device(torch.tensor(1e6, device=device))
    penalty_P_mu_3 = torch.where(P_mu_3 > 0, to_device(torch.tensor(0.0, device=device, device=device), to_device(torch.tensor(1e6, device=device))
    penalty_mu2_mu3 = torch.where(mu_2 > mu_3, to_device(torch.tensor(0.0, device=device, device=device), to_device(torch.tensor(1e6, device=device))
