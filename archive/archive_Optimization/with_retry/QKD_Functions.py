...
# def calculate_eta_ch_cached(L, alpha):
...
# # Define the optimized function for calculating detection probabilities
# @jit
# def calculate_D_mu_k(mu_k, eta_sys_values, P_dc):
#     return 1 - (1 - 2 * P_dc) * jnp.exp(-eta_sys_values * mu_k)
...
#     """
#     mu_values = jnp.array([mu_1, mu_2, mu_3])
#     P_n_given_mu = (mu_values**n_event * jnp.exp(-mu_values)) / calculate_factorial(n_event)
...
#     D_mu_1 = calculate_D_mu_k(mu_1, eta_sys_value, P_dc)
#     D_mu_2 = calculate_D_mu_k(mu_2, eta_sys_value, P_dc)
#     D_mu_3 = calculate_D_mu_k(mu_3, eta_sys_value, P_dc)
...
#     D_mu_k_values = jnp.array(D_mu_k_values)

#     # Compute terms safely with clipping to avoid numerical instability
#     terms = jnp.clip(p_mu_k_values * D_mu_k_values * P_X_value**2, 1e-12, None)
    

#     # Compute N_value safely to avoid division by zero
#     sum_terms = jnp.sum(terms)
#     N_value = jnp.where(sum_terms > 0, n_X / sum_terms, jnp.inf)  # Avoid divide by zero
...
#     """
#     epsilon_sec = jnp.clip(epsilon_sec, 1e-12, None)  # Avoid log(0)
...
#     # Ensure inputs are JAX arrays for compatibility
#     tau_1, mu_1, mu_2, mu_3, n_minus_mu_2, n_plus_mu_3, n_plus_mu_1, s_0, tau_0 = \
#         map(jnp.asarray, (tau_1, mu_1, mu_2, mu_3, n_minus_mu_2, n_plus_mu_3, n_plus_mu_1, s_0, tau_0))


#     numerator = tau_1 * mu_1 * (n_minus_mu_2 - n_plus_mu_3 - jnp.clip(((mu_2**2 - mu_3**2) / mu_1**2) * 
#                             (n_plus_mu_1 - s_0 / jnp.clip(tau_0, 1e-12, None)), -1e12, 1e12))
#     denominator = jnp.clip(mu_1 * (mu_2 - mu_3) - mu_2**2 + mu_3**2, 1e-12, None)
...
#     P_Z_value =  jnp.clip(1 - P_X_value, 1e-6, 1)
...
#     # in_axes=(0, None):
# 	# •	0 means L_values should be mapped element-wise.
# 	# •	None keeps alpha constant across all computations.
#     # •	This allows JAX to apply the calculation across all L_values in parallel, reducing Python loop overhead.
#     # Precompute eta_ch_values with JIT optimized operations
#     alpha_scale = -alpha / 10
#     eta_ch_values = 10 ** (alpha_scale * L_values)  
#     # eta_ch_values = vmap(calculate_eta_ch, in_axes=(0, None))(L_values, alpha)
...
#     D_mu_k_values = vmap(calculate_D_mu_k, in_axes=(0, None, None))(mu_k_values, eta_sys_values, P_dc_value)
#     e_mu_k_values = vmap(calculate_e_mu_k, in_axes=(None, None, None, 0, None, 0))(P_dc_value, e_mis, P_ap, D_mu_k_values, eta_sys_values, mu_k_values)
...
# # e_obs_X_values = calculate_e_obs(m_X_total, 
#     e_obs_X_values = jnp.minimum(m_X_total / n_X_total, 0.5)
...
# @jit
# def validate_and_retry(params, max_retries=10):
#     """
#     Validates the given parameters and retries optimization if conditions are not met.

#     Parameters:
#     - params (list): The parameters to validate.
#     - max_retries (int): Maximum number of retries if conditions fail.

#     Returns:
#     - list: Validated parameters.
#     """
#     retries = 0
#     while retries < max_retries:
#         mu_1, mu_2, P_mu_1, P_mu_2, P_X_value = params
#         mu_3 = max(1 - P_mu_1 - P_mu_2, 1e-6)
        
#         # Conditions check
#         conditions_met = (mu_1 > mu_2 + mu_3) and (mu_2 / mu_1 < 1)
#         valid_sum = abs(P_mu_1 + P_mu_2 + mu_3 - 1) < 1e-6
#         valid_probabilities = (P_mu_1 > 0) and (P_mu_2 > 0) and (mu_2 > mu_3)

#         if conditions_met and valid_sum and valid_probabilities:
#             print(f"Parameters validated successfully on attempt {retries + 1}")
#             return params
#         else:
#             print(f"Attempt {retries + 1}: Conditions not met, retrying...")
#             # Randomly perturb parameters for retry
#             params = [
#                 np.random.uniform(0.1, 0.9),  # mu_1
#                 np.random.uniform(0.05, params[0]),  # mu_2 < mu_1
#                 np.random.uniform(0.1, 0.9),  # P_mu_1
#                 np.random.uniform(0.05, 1 - params[2]),  # P_mu_2 < 1 - P_mu_1
#                 np.random.uniform(0.05, 0.95)  # P_X_value
#             ]
#             retries += 1

#     print("Maximum retries reached. Returning last attempt parameters.")
#     return params  # Return the last attempted values even if they don't satisfy conditions

# def objective_with_validation(params, *args):
#     """
#     Objective function with validation and retries.

#     Parameters:
#     - params (list): The parameters to optimize.

#     Returns:
#     - float: Optimized key rate or penalty.
#     """
#     validated_params = validate_and_retry(params)
#     result = objective(validated_params, *args)
#     return result


# def is_valid_param_set(mu_1, mu_2, P_mu_1, P_mu_2, P_X_value):
#     mu_3 = max(1 - (P_mu_1 + P_mu_2), 1e-6)
#     return (mu_1 > mu_2 + mu_3) and (mu_2 / mu_1 < 1) and (P_mu_1 + P_mu_2 + mu_3 - 1 < 1e-6)
...
#         if len(params) != 5:
#             raise ValueError("Expected 5 parameters (mu_1, mu_2, P_mu_1, P_mu_2, P_X_value)")
        
#         mu_1, mu_2, P_mu_1, P_mu_2, P_X_value = params
#         mu_3 = 2e-4  # Fixed third intensity level
#         P_mu_3 = max(1 - P_mu_1 - P_mu_2, 1e-6)  # Ensure non-negative probability
        
#         # Compute key rate metrics
#         key_rates, eta_ch_values, S_X_0_values, S_Z_0_values, S_X_1_values, S_Z_1_values, \
#         tau_0_values, tau_1_values, e_mu_k_values, e_obs_X_values, v_Z_1_values, gamma_results, \
#         Phi_X_values, binary_entropy_Phi_values, lambda_EC_values, l_calculated_values = (
#             calculate_key_rates_and_metrics(params, L_values, n_X, alpha, eta_Bob, 
#                                             P_dc_value, epsilon_sec, epsilon_cor, f_EC, 
#                                             e_mis, P_ap, n_event)
#         )
...
#         return (
#     key_rates, eta_ch_values, S_X_0_values, S_Z_0_values, 
...
#         return float('inf')  # Return an extreme penalty in case of failure

# @jit
# def optimized_objective(params, L_values, n_X, alpha, eta_Bob, P_dc_value, epsilon_sec, epsilon_cor, f_EC, e_mis, P_ap, n_event):
#     return objective(params, L_values, n_X, alpha, eta_Bob, P_dc_value, epsilon_sec, epsilon_cor, f_EC, e_mis, P_ap, n_event)
  
# # # Apply JIT compilation after defining the function
# # optimized_objective = jit(objective, static_argnums=(1,))
# # Example usage in optimization

# def initialize_parameters():
#     params = [0.5, 0.2, 0.3, 0.2, 0.7]  # Initial guess values
# # initial_params = [0.2, 0.1, 0.4, 0.3, 0.5]  # More balanced initialization
#     L_values = np.linspace(0.1, 200, 1000)  # Fiber lengths
#     n_X = int(1e8)  # Convert to int for clarity
#     alpha = 0.2  # Attenuation coefficient
#     eta_Bob = 0.1  # Bob's detector efficiency
#     P_dc_value = 6e-7  # Dark count probability
#     epsilon_sec = 1e-10  # Secrecy error
#     epsilon_cor = 1e-15  # Correctness error
#     f_EC = 1.16  # Error correction efficiency
#     e_mis = 5e-3  # Misalignment error
#     P_ap = 0.0  # After-pulse probability
#     n_event = 1  # Single-photon event
    
#     return params, L_values, n_X, alpha, eta_Bob, P_dc_value, epsilon_sec, epsilon_cor, f_EC, e_mis, P_ap, n_event
...
# Misalignment error probability
    kappa = 1e-15           # given in the papeer
    return
