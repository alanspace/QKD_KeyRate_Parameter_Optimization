...
#e_2
P_dc_value = torch.tensor(6e-7, device=torch.device("mps"))   # Dark count probability
Y_0 = P_dc_value
# 2.7*10** -7
...
e_mis = torch.tensor(5e-3, device=torch.device("mps"))  # Misalignment error probability # given in the paper, discussed with range from 0 to 0.1 
e_d = e_mis
# 0.026 
e_3 = e_d * 100
...
# e_4
# Detected events
n_X_values = torch.tensor([10 ** s for s in range(6, 11)], dtype=torch.float32, device=torch.device("mps"))  # Detected events # Detected events
...
# %%
alpha = torch.tensor(0.2, device=torch.device("mps"))  # Attenuation coefficient (dB/km), given in the paper
eta_Bob = torch.tensor(0.1, device=torch.device("mps"))  # Detector efficiency, given in the paper
P_ap = torch.tensor(0.0, device=torch.device("mps"))  # After-pulse probability
f_EC = torch.tensor(1.16, device=torch.device("mps"))  # Error correction efficiency given in the paper, range around 1.1
epsilon_sec = torch.tensor(1e-10, device=torch.device("mps"))  # Security error # is equal to kappa * secrecy length Kl, range around 1e-10 Scalar, as it is a single value throughout the calculations.
epsilon_cor = torch.tensor(1e-15, device=torch.device("mps"))  # Correlation error # given in the paper, discussed with range from 0 to 10e-10
# Dark count probability
n_event = torch.tensor(1, device=torch.device("mps"))  # For single-photon events
kappa = torch.tensor(1e-15, device=torch.device("mps"))  # Security parameter given in the paper
...
Ls = torch.linspace(0.1, 220, 1000, device=torch.device("mps"), dtype=torch.float32)  # Fiber lengths
...
for metric, (ylabel, title) in metric_info.items():
    ...
    for n_X, data in results.items():
        if metric in data and "fiber_lengths" in data:  # Check both keys exist
            fiber_lengths = torch.tensor(data["fiber_lengths"], dtype=torch.float32)
            metric_values = torch.tensor(data[metric], dtype=torch.float32)
