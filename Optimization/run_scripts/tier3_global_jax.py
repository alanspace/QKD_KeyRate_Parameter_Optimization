...
import plot_results as plot_final_results
...
SPLIT_SEGMENTS_MAP = {
    1e4: 3,  # Split 10^4 (50 min job) into 3 segments (approx 17 min each)
    1e5: 2,  # Split 10^5 (41 min job) into 2 segments (approx 21 min each)
    1e6: 1,  # Keep sequential
    1e7: 1,
    1e8: 1,
    1e9: 1
}
...
def run_single_nx_curve(n_X, L_values, bounds, constants):
    ...
    num_segments = SPLIT_SEGMENTS_MAP.get(n_X, 1)
    
    # Split L_values into chunks
    L_chunks = np.array_split(L_values, num_segments)
    
    final_results = []
    total_sequential_time_sum = 0
    
    # --- Execute Segments in Parallel ---
    # We use a NEW ProcessPoolExecutor here dedicated to the segments of the current n_X.
    # This is "Nested Parallelism" (Pool inside Pool).
    # Since the outer pool spawns Processes, and this spawns Processes, we must be careful.
    # However, Python's multiprocessing handles this via spawn context usually.
    # Given we have 12 cores and only split 10^4 (3) and 10^5 (2), total active workers = 3+2+1+1+1+1 = 9 < 12.
    # It fits perfectly.
    
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_segments) as executor:
        futures = []
        
        for i, L_chunk in enumerate(L_chunks):
            # 1. Determine the Cold Start for this segment
            # We restart continuity for each segment using the Table Guess.
            # This is acceptable because the table is accurate enough to seed the smooth search.
            initial_P_start = INITIAL_GUESSES_MAP.get(n_X, DEFAULT_GUESS)
            
            # 2. Submit the job
            future = executor.submit(
                _run_sequential_segment, 
                n_X, L_chunk, bounds, constants, initial_P_start, f"n_X={n_X:.0e}-SEG-{i+1}/{num_segments}"
            )
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                segment_results, segment_duration = future.result()
                final_results.extend(segment_results)
                total_sequential_time_sum += segment_duration
            except Exception as e:
                logging.error(f"FATAL ERROR during segment collection for n_X={n_X:.0e}: {e}")
                # We do not raise immediately to allow other segments to maybe finish? 
                # No, if one segment fails, the curve is broken.
                raise e 

    # --- Final Stitching ---
    # Sort results by L (critical since parallel return order is non-deterministic)
    final_results.sort(key=lambda x: x["fiber_length"])
    
    # We return the WALL clock duration of this curve (approx max of segments + overhead),
    # but for the report "Total Sequential Time", we usually want the sum.
    # The main loop aggregates 'duration'. Let's return the WALL time here to reflect parallel speedup in logs.
    duration_wall = time.time() - start_time_total

    return n_X, final_results, duration_wall

def run_jax_pipeline():
    # Pre-calculate Run Directory
    script_dir_out = os.path.dirname(os.path.abspath(__file__))
    base_output_dir = os.path.join(os.path.dirname(script_dir_out), "outputs", "global_jax")
    ...
    try:
        plot_final_results.generate_plots(filepath, filename_prefix="GlobalJAX_")
    except Exception as e:
